...
if (props.routes && props.routes.length > 0) {
  props.routes.forEach(route => {
    option.series.push({
      ...
      polyline: true,
      data: [{
        coords: route.coordinate.flat(), // Flatten one level to [[lng, lat], [lng, lat], ...]
        name: route.name,
      }],
      lineStyle: {
        color: route.lineStyle?.color || '#ff0000',
        width: route.lineStyle?.width || 2,
        type: route.lineStyle?.type || 'solid'
      },
      tooltip: route.tooltip || {
        formatter: route.name
      },
      effect: {
        show: false // Set to true if you want moving effects
      }
    })
  })
}
